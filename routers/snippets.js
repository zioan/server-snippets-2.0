...
router.put('/update/:id', auth, (req, res) => {
  ...
  // const sql = `UPDATE snippets SET user_id = '${newData.user_id}', title = '${newData.title}', tag = '${newData.tag}', code = '${newData.code}' WHERE id = ${req.params.id}`

  // db.query(sql, (err, result) => {
  const sql = `UPDATE snippets SET user_id = ?, title = ?, tag = ?, code = ? WHERE id = ?`

  db.query(
    sql,
    [newData.user_id, newData.title, newData.tag, newData.code, req.params.id],
    (err, result) => {
      if (err) {
        console.log(err)
        return res.json({ message: err })
